...
def unproject_depth_map_to_point_map_torch(
    ...
    """
    # Ensure depth_map has shape (S, H, W)
    if depth_map.dim() == 4 and depth_map.shape[-1] == 1:
        depth_map = depth_map.squeeze(-1)  # Remove last dimension if it exists
    ...
    pixel_coords = torch.stack([x, y, ones], dim=-1)  # Shape: (H, W, 3)

    # Reshape pixel_coords to (H*W, 3)
    pixel_coords = pixel_coords.view(-1, 3).unsqueeze(0)  # Shape: (1, H*W, 3)

    # Repeat pixel_coords for each sample in the batch
    pixel_coords = pixel_coords.expand(S, -1, -1)  # Shape: (S, H*W, 3)

    # Normalize pixel coordinates using intrinsic matrix
    inv_intrinsics = torch.inverse(intrinsics_cam)  # Shape: (S, 3, 3)
    cam_coords = torch.bmm(inv_intrinsics, pixel_coords.permute(0, 2, 1))  # Shape: (S, 3, H*W)

    # Scale by depth values
    depth_values = depth_map.view(S, -1)  # Shape: (S, H*W)
    cam_coords = cam_coords * depth_values.unsqueeze(1)  # Shape: (S, 3, H*W)

    # Transform to world coordinates using extrinsics
    world_coords = torch.bmm(extrinsics_cam[:, :, :3], cam_coords) + extrinsics_cam[:, :, 3:]  # Shape: (S, 3, H*W)

    # Reshape world_coords to (S, H, W, 3)
    world_coords = world_coords.permute(0, 2, 1).view(S, H, W, 3)
