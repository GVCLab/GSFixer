...
parser.add_argument("--scene_name", type=str, default="bonsai")
...
output_scene_root = os.path.join(output_root, scene_name)

for n_views in n_views_list:
    result_dir = os.path.join(output_scene_root, str(n_views))
    train_cmd = (
    ...
    render_cmd = f"CUDA_VISIBLE_DEVICES={args.device_id} python render.py -m {result_dir} --data_dir {scene_path}"
    print(render_cmd)
    os.system(render_cmd)
    print("finish render cmd", scene_name, n_views)


    metrics_cmd = f"CUDA_VISIBLE_DEVICES={args.device_id} python metrics.py -m {result_dir}"
    print(metrics_cmd)
