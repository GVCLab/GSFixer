...
class GaussianExtractor(object):
    ...
    def export_video(self, path):
        ...
        pil_frames = [Image.fromarray(frame) if isinstance(
            frame, np.ndarray) else frame for frame in renderings]
        pil_frames[0].save(os.path.join(path, "rgb_render.mp4").replace('.mp4', '.gif'),
                            format='GIF',
        ...
        pil_frames = [Image.fromarray(frame) if isinstance(
            frame, np.ndarray) else frame for frame in renderings]
        pil_frames[0].save(os.path.join(path, "rgb_gt.mp4").replace('.mp4', '.gif'),
                            format='GIF',
                            append_images=pil_frames[1:],
                            save_all=True,
                            duration=500,
