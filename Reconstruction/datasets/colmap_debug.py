    ...
    def __init__(
        ...
        if self.args.outpaint_type == "sparse" and sparse_view > 0:
            ...
            if os.path.exists(split_file):
                ...
                train_indices = [c for idx, c in enumerate(indices) if idx % args.dataset.test_every != 0]
                test_indices = [c for idx, c in enumerate(indices) if idx % args.dataset.test_every == 0]

                if sparse_view > 0:
                    idx_sub = np.linspace(0, len(train_indices) - 1, sparse_view)
                    idx_sub = [round(i) for i in idx_sub]
                    train_indices = [train_indices[i] for i in idx_sub]
